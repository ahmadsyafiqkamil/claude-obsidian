...
import { summarizeResult } from "../log-summary.js";
import fs from "fs";
...
const toolMap = {
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      softStopEnabled: ["management", "softStopEnabled"],
      softStopPct: ["management", "softStopPct"],
      softStopMinAgeMinutes: ["management", "softStopMinAgeMinutes"],
      // risk
      maxPositions: ["risk", "maxPositions"],
      maxDeployAmount: ["risk", "maxDeployAmount"],
      // schedule
      managementIntervalMin: ["schedule", "managementIntervalMin"],
      managementIntervalActiveMin: ["schedule", "managementIntervalActiveMin"],
    ...
    const intervalChanged = applied.managementIntervalMin != null || applied.managementIntervalActiveMin != null || applied.screeningIntervalMin != null || applied.pnlPollIntervalSec != null;
    if (intervalChanged && _cronRestarter) {
      _cronRestarter();
      log("config", `Cron restarted — management base: ${config.schedule.managementIntervalMin}m / active: ${config.schedule.managementIntervalActiveMin}m, screening: ${config.schedule.screeningIntervalMin}m`);
    }
    ...
    const lessonsKeys = Object.keys(applied).filter(
      k => !k.startsWith("_") && k !== "managementIntervalMin" && k !== "managementIntervalActiveMin" && k !== "screeningIntervalMin"
    );
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        // Carry the actual measured rate into the note, and skip the note
        // entirely when the reason claims low yield but the measured rate is
        // actually at/above threshold (guards against stale LLM claims).
        if (args.reason && args.reason.toLowerCase().includes("low yield")) {
          const poolAddr = result.pool || args.pool_address;
          const rateMatch = args.reason.match(/fee rate ([\d.]+)%\/24h/);
          const measuredRate = rateMatch ? parseFloat(rateMatch[1]) : null;
          const minRate = config.management.minFeePerTvl24h;
          const rateContradictsLowYield = measuredRate != null && minRate != null && measuredRate >= minRate;
          if (poolAddr && !rateContradictsLowYield) {
            const rateNote = measuredRate != null ? ` — actual ${measuredRate}%/24h` : "";
            addPoolNote({ pool_address: poolAddr, note: `Closed: low yield${rateNote} at ${new Date().toISOString().slice(0,10)}` }).catch?.(() => {});
          }
        }
...
// summarizeResult imported from ../log-summary.js
