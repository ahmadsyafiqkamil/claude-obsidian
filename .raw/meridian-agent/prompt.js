...
/**
 * Human-readable momentum→bins table for the SCREENER prompt, built from
 * config.momentum.classes. Strong momentum → tighter (fewer) bins.
 */
function buildMomentumBinsGuidance() {
  const classes = config.momentum?.classes || [];
  // Clamp each band to the executor's safety floor/ceiling so the guidance can never
  // instruct a bins_below value that the deploy safety check would reject (regression
  // insurance: keeps bands valid if minBinsBelow/maxBinsBelow are later changed alone).
  ...
  return classes
    .map((c, i) => {
      const prevMin = i > 0 ? classes[i - 1].min : Infinity;
      const label = c.min === -Infinity || !Number.isFinite(c.min)
        ? `1h < ${Number.isFinite(prevMin) ? prevMin : "?"}%`
        : `1h ≥ ${c.min}%`;
      return `${c.name} (${label}) → ${clamp(c.bins.min)}-${clamp(c.bins.max)} bins`;
    })
    .join("; ");
}
...
- bins_below: ${config.momentum?.binsEnabled
  ? `MOMENTUM-PRIMARY. Each candidate has pre-computed momentum.category and momentum.bins. Set bins_below within that band, biasing to the LOWER (tighter) end when momentum is stronger and the UPPER (wider) end when weaker — strong trends pull back less. Bands: ${buildMomentumBinsGuidance()}. If a candidate's momentum is unavailable, fall back to round(${config.strategy.minBinsBelow} + (volatility/5)*${config.strategy.maxBinsBelow - config.strategy.minBinsBelow}) clamped to [${config.strategy.minBinsBelow},${config.strategy.maxBinsBelow}]`
  : `round(${config.strategy.minBinsBelow} + (candidate volatility/5)*${config.strategy.maxBinsBelow - config.strategy.minBinsBelow}) clamped to [${config.strategy.minBinsBelow},${config.strategy.maxBinsBelow}]`}. bins_above = 0.
