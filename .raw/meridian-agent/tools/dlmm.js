...
import { normalizeStrategy } from "./strategy-normalize.js";
...
import { normalizeMint, invalidateBalancesCache } from "./wallet.js";
import { appendDecision } from "../decision-log.js";
import { getAndClearStagedSignals } from "../signal-tracker.js";
import { computePositions, fetchDlmmPnlForPool } from "./pnl.js";
import { resolveYieldRate } from "../yield-metric.js";
...
export async function deployPosition({
  ...
  pool_address = normalizeMint(pool_address);
  const activeStrategy = normalizeStrategy(strategy || config.strategy.strategy);
  ...
  if (shouldUseLpAgentRelayForDeploy()) {
    try {
      ...
      _positionsCacheAt = 0;
      invalidateBalancesCache(); // SOL balance changed after deploy
...
export async function getPositionPnl({ pool_address, position_address }) {
    ...
    try {
      ...
      if (p) {
        return {
          ...
          fee_rate_24h: p.fee_rate_24h ?? null,
          fee_rate_24h_source: p.fee_rate_24h_source ?? null,
          in_range: p.in_range,
  ...
  try {
    ...
    return {
      ...
      fee_per_tvl_24h:   maybeNum(p.feePerTvl24h) != null ? Math.round(maybeNum(p.feePerTvl24h) * 100) / 100 : null,
      fee_rate_24h:      (() => {
        const deposit = maybeNum(p.allTimeDeposits?.total?.usd);
        const { rate } = resolveYieldRate({
          unclaimed_fees_usd: safeNum(p.unrealizedPnl?.unclaimedFeeTokenX?.usd) + safeNum(p.unrealizedPnl?.unclaimedFeeTokenY?.usd),
          collected_fees_usd: safeNum(p.allTimeFees?.total?.usd),
          deposit_usd: deposit != null && deposit > 0 ? deposit : null,
          age_minutes: p.createdAt ? Math.floor((Date.now() - p.createdAt * 1000) / 60000) : null,
          fee_per_tvl_24h: maybeNum(p.feePerTvl24h),
        });
  ...
  const loadPositions = async () => { try {
    ...
    for (const pool of pools) {
      for (const positionAddress of (pool.listPositions || [])) {
        ...
        const depositUsdRaw = binData ? maybeNum(binData.allTimeDeposits?.total?.usd) : null;
        const shaped = {
          position:           positionAddress,
          ...
          unclaimed_fees_true_usd: lpData
            ...
            : null,
          fee_per_tvl_24h:    (() => {
            const v = binData ? maybeNum(binData.feePerTvl24h) : null;
            return v != null ? Math.round(v * 100) / 100 : null;
          })(),
          deposit_usd:        depositUsdRaw != null && depositUsdRaw > 0 ? Math.round(depositUsdRaw * 10000) / 10000 : null,
        ...
        const { rate: feeRate24h, source: feeRateSource } = resolveYieldRate({
          unclaimed_fees_usd: shaped.unclaimed_fees_true_usd,
          collected_fees_usd: shaped.collected_fees_true_usd,
          deposit_usd: shaped.deposit_usd,
          age_minutes: shaped.age_minutes,
          fee_per_tvl_24h: shaped.fee_per_tvl_24h,
        });
        positions.push({
          ...shaped,
          fee_rate_24h: feeRate24h != null ? Math.round(feeRate24h * 100) / 100 : null,
          fee_rate_24h_source: feeRateSource,
        });
      }
    }

    // ── On-chain ghost-position filter ──────────────────────────────
    // Meteora portfolio API can return stale entries (positions already
    // closed via UI or external tx). Without this check the bot loops
    // close_position → 0xbbf AccountOwnedByWrongProgram indefinitely,
    // because the SPL account was closed and ownership reverted to
    // System Program. Verify each position still exists and is owned
    // by the DLMM program; drop ghosts and let syncOpenPositions
    // auto-close the local state.
    let ghostFiltered = 0;
    try {
      const allAddresses = positions.map(p => new PublicKey(p.position));
      if (allAddresses.length > 0) {
        const dlmmProgram = getDlmmProgramId();
        const infos = await getConnection().getMultipleAccountsInfo(allAddresses, "confirmed");
        const verified = [];
        for (let i = 0; i < positions.length; i++) {
          const info = infos[i];
          if (!info || !info.owner) {
            ghostFiltered++;
            log("positions_warn", `Ghost position filtered: ${positions[i].position.slice(0,8)} (account not found on-chain — likely closed externally)`);
            continue;
          }
          if (!info.owner.equals(dlmmProgram)) {
            ghostFiltered++;
            log("positions_warn", `Ghost position filtered: ${positions[i].position.slice(0,8)} (owner=${info.owner.toBase58()} ≠ DLMM program; account was closed and ownership reverted to System Program or other)`);
            continue;
          }
          verified.push(positions[i]);
        }
        if (ghostFiltered > 0) {
          positions.length = 0;
          positions.push(...verified);
          log("positions_warn", `Dropped ${ghostFiltered} ghost position(s) after on-chain verification; ${positions.length} live position(s) remain.`);
        }
      }
    } catch (err) {
      // Don't block the loop on RPC blips; log and continue.
      log("positions_warn", `Ghost-filter RPC check failed (${err.message}); proceeding with unverified list`);
    }
    ...
    const result = {
      ...
      positions,
      source: "meteora",
      ghost_filtered_count: ghostFiltered,
    };
    if (useLocalWallet) {
...
export async function claimFees({ position_address }) {
  ...
  try {
    ...
    invalidateBalancesCache(); // claimed fees arrived as SOL
...
export async function closePosition({ position_address, reason }) {
  ...
  try {
    ...
    // -- Ghost guard ------------------------------------------------------
    // If the position account no longer exists on-chain (closed via UI or an
    // external tx), skip the close entirely. Otherwise the SDK simulation
    // fails with custom error 3007 and the management loop retries forever.
    try {
      const ghostInfo = await getConnection().getAccountInfo(new PublicKey(position_address), "confirmed");
      if (!ghostInfo || !ghostInfo.owner || !ghostInfo.owner.equals(getDlmmProgramId())) {
        log("close", `Ghost close skipped: ${position_address} is not a live DLMM account on-chain - clearing local state.`);
        recordClose(position_address, reason || "ghost - closed externally");
        _positionsCacheAt = 0;
        return { success: true, ghost: true, position: position_address, message: "Position already closed on-chain; cleared from local state." };
      }
    } catch (ghostErr) {
      log("close_warn", `Ghost-guard check failed (${ghostErr.message}); proceeding with normal close.`);
    }
    const wallet = getWallet();
    const poolAddress = await lookupPoolForPosition(position_address, wallet.publicKey.toString());
    ...
    _positionsCacheAt = 0;
    invalidateBalancesCache(); // SOL balance changed after close
