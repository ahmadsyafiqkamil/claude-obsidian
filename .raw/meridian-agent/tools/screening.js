...
import { isBaseMintOnCooldown, isPoolOnCooldown, getLastCloseForMint, getPoolPerformanceBlock } from "../pool-memory.js";
import { confirmIndicatorPreset } from "./chart-indicators.js";
import { discoverGmgnPools } from "./gmgn.js";
import { getMomentumData } from "./dexscreener.js";
import { classifyMomentum, momentumToBins, shouldAllowReentry } from "../momentum.js";
...
function getRawPoolScreeningRejectReason(pool, s) {
  ...
  if (s.minVolumeChangePct != null && volumeChangePct != null && volumeChangePct < s.minVolumeChangePct) {
    return `volume decelerating (volume_change_pct ${volumeChangePct} < min ${s.minVolumeChangePct})`;
  }
  if (
    pool?.discord_signal &&
...
export async function discoverPools({
  ...
  const filters = [
    ...
    s.minVolumeChangePct != null ? `volume_change_pct>=${s.minVolumeChangePct}` : null,
...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  const eligible = pools
    .filter((p) => {
      ...
      const perfBlock = getPoolPerformanceBlock(p.pool);
      if (perfBlock.blocked) {
        log("screening", `Filtered poor-history pool ${p.name} (${p.pool?.slice(0, 8)}): ${perfBlock.reason}`);
        pushFilteredReason(filteredOut, p, perfBlock.reason);
        return false;
      }
      return true;
    })
  ...
  // ── Momentum: enrich (DexScreener), entry filter, dynamic re-entry gate ──
  if (config.momentum.enabled && eligible.length > 0) {
    const m = config.momentum;
    const now = Date.now();
    const momentumData = await Promise.all(
      eligible.map((p) => getMomentumData(p.base?.mint, p.pool).catch(() => null)),
    );
    const before = eligible.length;
    const kept = [];
    eligible.forEach((pool, i) => {
      const data = momentumData[i] || { available: false };
      const category = data.available ? classifyMomentum(data.h1, m.classes) : null;
      // Clamp the band to the executor's safety floor/ceiling so a candidate can never
      // carry a bins suggestion the deploy safety check would reject (regression insurance).
      const rawBins = category ? momentumToBins(category, m.classes) : null;
      const floor = config.strategy?.minBinsBelow ?? 0;
      const ceil = config.strategy?.maxBinsBelow ?? Infinity;
      const bins = rawBins
        ? { min: Math.max(floor, Math.min(ceil, rawBins.min)), max: Math.max(floor, Math.min(ceil, rawBins.max)) }
        : null;
      // Mutating the candidate matches this pipeline's idiom (see indicator_confirmation below).
      pool.momentum = {
        h1: data.h1 ?? null,
        m5: data.m5 ?? null,
        h6: data.h6 ?? null,
      ...
      if (!data.available) {
        if (m.failOpen) { kept.push(pool); return; }
        pushFilteredReason(filteredOut, pool, "momentum data unavailable (fail-closed)");
        return;
      }

      // Entry filter — require positive price-change momentum above threshold
      if (m.entryMinH1Pct != null && (pool.momentum.h1 == null || pool.momentum.h1 < m.entryMinH1Pct)) {
        pushFilteredReason(filteredOut, pool, `1h momentum ${pool.momentum.h1 ?? "?"}% < ${m.entryMinH1Pct}%`);
        return;
      }
      if (m.entryMinM5Pct != null && (pool.momentum.m5 == null || pool.momentum.m5 < m.entryMinM5Pct)) {
        pushFilteredReason(filteredOut, pool, `5m momentum ${pool.momentum.m5 ?? "?"}% < ${m.entryMinM5Pct}%`);
        return;
      }

      // Dynamic re-entry gate for previously-traded tokens
      if (m.reentryEnabled) {
        const lastClose = getLastCloseForMint(pool.base?.mint);
        if (lastClose?.closedAt) {
          const hoursSinceClose = (now - new Date(lastClose.closedAt).getTime()) / 3_600_000;
          const decision = shouldAllowReentry({
            hoursSinceClose,
            priceChange1h: pool.momentum.h1,
            rules: m.reentryRules,
          });
          if (!decision.allow) {
            log("screening", `Re-entry blocked ${pool.name}: ${decision.reason}`);
    ...
    if (eligible.length < before) {
      log("screening", `Momentum filter removed ${before - eligible.length} candidate(s)`);
    }
  }
