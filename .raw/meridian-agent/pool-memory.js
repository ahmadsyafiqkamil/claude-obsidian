...
export function recordPoolDeploy(poolAddress, deployData) {
  ...
  // Dynamic re-entry (momentum.js) supersedes the static time-based cooldowns below.
  // When it's enabled, skip low-yield/repeat-fee cooldowns so the momentum gate decides.
  // The repeated-OOR cooldown always applies — it's a safety guard, not a yield throttle.
  const dynamicReentry = config.momentum?.reentryEnabled ?? false;

  // Set cooldown for low yield closes — pool wasn't profitable enough, don't redeploy soon
  if (!dynamicReentry && deploy.close_reason === "low yield") {
  ...
  if (!dynamicReentry && config.management.repeatDeployCooldownEnabled) {
...
/**
 * Hard-filter gate based on this pool's own track record.
 * Reads thresholds from config.screening. Returns { blocked, reason }.
 *
 * Two conditions (either triggers a block):
 *  1. Win-rate gate  — total_deploys >= minDeploysForWinRateGate AND win_rate < minPoolWinRate
 *  2. Consecutive-loss gate — last maxConsecutiveLosses closes are all losses
 *
 * Pool with no prior history is always allowed (returns blocked:false).
 * Can be disabled entirely by setting poolHistoryFilterEnabled=false in user-config.json.
 */
export function getPoolPerformanceBlock(poolAddress) {
  if (!poolAddress) return { blocked: false, reason: null };
  const s = config.screening;
  if (s.poolHistoryFilterEnabled === false) return { blocked: false, reason: null };

  const db = load();
  const entry = db[poolAddress];
  if (!entry || !Array.isArray(entry.deploys) || entry.deploys.length === 0) {
    return { blocked: false, reason: null }; // pool baru → jangan dihukum
  }

  // Gate 1: win-rate (uses raw win_rate, not adjusted, so OOR losses count)
  const total = entry.total_deploys ?? entry.deploys.length;
  const winRate = entry.win_rate ?? 0; // fraction 0..1
  const minDeploys = s.minDeploysForWinRateGate ?? 3;
  const minWinRate = s.minPoolWinRate ?? 0.45;
  if (total >= minDeploys && winRate < minWinRate) {
    return {
      blocked: true,
      reason: `poor pool win-rate ${Math.round(winRate * 100)}% over ${total} deploys (< ${Math.round(minWinRate * 100)}%)`,
    };
  }

  // Gate 2: consecutive losses on most-recent closed deploys
  const maxLosses = s.maxConsecutiveLosses ?? 2;
  const closed = entry.deploys.filter((d) => d?.closed_at != null);
  if (maxLosses > 0 && closed.length >= maxLosses) {
    const recent = closed.slice(-maxLosses);
    if (recent.every((d) => (d.pnl_pct ?? 0) < 0)) {
      return { blocked: true, reason: `${maxLosses} consecutive losses on this pool` };
    }
  }
...
/**
 * Most recent close across all pools holding this base mint.
 * Used by the dynamic re-entry gate (momentum.js shouldAllowReentry).
 * @returns {{ closedAt: string, outcome: "profit"|"loss", pnlPct: number|null } | null}
 */
export function getLastCloseForMint(baseMint) {
  if (!baseMint) return null;
  const db = load();
  let latest = null;
  for (const entry of Object.values(db)) {
    ...
    for (const d of entry.deploys) {
      const t = d?.closed_at ? new Date(d.closed_at).getTime() : NaN;
      if (!Number.isFinite(t)) continue;
      if (!latest || t > latest.time) {
        latest = {
          time: t,
          closedAt: d.closed_at,
          outcome: (d.pnl_pct ?? 0) >= 0 ? "profit" : "loss",
          pnlPct: d.pnl_pct ?? null,
        };
      }
    }
  }
  return latest ? { closedAt: latest.closedAt, outcome: latest.outcome, pnlPct: latest.pnlPct } : null;
}
